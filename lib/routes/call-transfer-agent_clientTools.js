/*
  This example uses SIP REFER as a method to transfer the call to a human agent via Ultravox clientTools.
  Ensure that the carrier of your choice supports SIP REFER.

  https://docs.jambonz.org/verbs/verbs/sip-refer

  Alternatively, you can use the dial verb with INVITE as the SIP method. When opting for this method, 
  uncomment the commented code snippets, and remove the lines marked with "Remove ... when using dial instead of sip:refer"
...
const service = ({logger, makeService}) => {
  ...
  svc.on('session:new', (session, path) => {
    ...
    session
      ...
      .on('/sip_referAction', sip_referAction.bind(null, session))  // Remove when using dial instead of sip:refer
      .on('/sip_referEvent', sip_referEvent.bind(null, session))  // Remove when using dial instead of sip:refer
...
// Remove when using dial instead of sip:refer
